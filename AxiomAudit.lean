...
1. a **tracked-tree census** — over every *persisted* declaration whose defining
   ...
   *only* custom axiom declaration in the tracked tree — a new stray `axiom`
   ...
   modules on disk equals the set imported here *and* equals a pinned 55-name
...
run_cmd do
  ...
  -- equally), so the tracked surface is additionally pinned to this exact 55-name
  ...
  logInfo m!"Census PASS: {diskMods.length} tracked RadicalRelativity modules (== frozen 55-name manifest), custom axioms exactly {citedAxioms}, every tracked persisted declaration's closure ⊆ {allowed}"
