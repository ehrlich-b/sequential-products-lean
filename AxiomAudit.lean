...
1. a **tracked-tree census** — over every *persisted* declaration whose defining
   ...
   modules on disk equals the set imported here *and* equals a pinned 166-name
...
run_cmd do
  ...
  let isProject := fun (n : Name) =>
    ...
    | none => false
  -- (a) closure census + custom-axiom collection.
  -- v4.33: `Lean.CollectAxioms.State`/`.collect` are no longer exposed; the public entry point is
  -- `Lean.collectAxioms : Name -> m (Array Name)`, which is used here instead. Same census,
  -- same allowed set; only the plumbing changed.
  ...
  -- equally), so the tracked surface is additionally pinned to this exact 166-name
  ...
  let expectedMods : List Name :=
    [`RadicalRelativity,
     ...
     `RadicalRelativity.DirectSum,
     `RadicalRelativity.EJA.Peirce,
     `RadicalRelativity.EJA.PeirceMul,
     `RadicalRelativity.EJA.Orthogonal,
     `RadicalRelativity.EJA.Frame,
     `RadicalRelativity.EJA.Power,
     `RadicalRelativity.EJA.PowerAssoc,
     `RadicalRelativity.EJA.FormallyReal,
     `RadicalRelativity.EJA.Subalgebra,
     `RadicalRelativity.EJA.Block,
     `RadicalRelativity.EJA.Pattern,
     `RadicalRelativity.EJA.Bridge,
     `RadicalRelativity.EJA.InterfaceInstance,
     `RadicalRelativity.EJA.ConcreteInstance,
     `RadicalRelativity.EJA.Witness,
     ...
     `RadicalRelativity.Hermitian.RCLikeGeneral,
     `RadicalRelativity.Hermitian.OperatorInstances,
     ...
     `RadicalRelativity.Necessity.BlockModelGen,
     `RadicalRelativity.Necessity.FrameBlockSpanGen,
     `RadicalRelativity.Necessity.CoalescenceDiffGen,
     `RadicalRelativity.Necessity.BlockInvarianceGen,
     `RadicalRelativity.Necessity.BlockTransportGen,
     `RadicalRelativity.Necessity.BlockChiGen,
     `RadicalRelativity.Necessity.ConjTransportGen,
     `RadicalRelativity.Necessity.RealRigidity,
     ...
     `RadicalRelativity.Necessity.RealRayMap,
     `RadicalRelativity.Necessity.RealRankOneSpan,
     `RadicalRelativity.Necessity.RealJordanWitness,
     `RadicalRelativity.Necessity.RealStrength,
     `RadicalRelativity.Necessity.RealProjectionOrder,
     `RadicalRelativity.Necessity.RealWignerBridge,
     `RadicalRelativity.Necessity.ComplexResidue,
     `RadicalRelativity.Necessity.FrameConstancy,
     `RadicalRelativity.Necessity.UnitaryGeneration,
     `RadicalRelativity.Necessity.ComplexRowUnconditional,
     `RadicalRelativity.Necessity.RealInducedMap,
     `RadicalRelativity.Necessity.RealKadison,
     `RadicalRelativity.Necessity.RealRowUnconditional,
     ...
     `RadicalRelativity.RankTwo.Descent,
     `RadicalRelativity.RankTwo.Bloch,
     `RadicalRelativity.RankTwo.Separation,
     `RadicalRelativity.RankTwo.Sufficiency,
     ...
     `RadicalRelativity.PaperA.CertifiedConfiguration,
     ...
     `RadicalRelativity.Necessity.OrderUnitS2,
  ...
  logInfo m!"Census PASS: {diskMods.length} tracked RadicalRelativity modules (== frozen 166-name manifest), custom axioms exactly {citedAxioms}, every tracked persisted declaration's closure ⊆ {allowed}"
