...
| `thm:qubit-boundary`(iii) **in the `twistSeq` encoding** — the article's `τ` family is not ANY constant twist product; closes the encoding split that left rows 30 and 31 speaking about two unlinked objects. ★ Scope: "not *literally* a constant twist"; the article's stronger `(Φ, t)`-conjugation form is NOT proved | `RankTwo.exists_n2Sp_tau_ne_twistSeq`, `RankTwo.exists_n2Sp_ne_twistSeq_of_nonconstant`, `RankTwo.exists_twistSeq_diagFamily_ne`, `RankTwo.surjective_colFrame` | `RankTwo/Sufficiency.lean` |
