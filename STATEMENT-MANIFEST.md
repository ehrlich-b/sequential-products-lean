...
**Count at ARC-6 rung 6.0/6.2 (2026-08-08): 7 FORMALIZED / 20 PARTIAL / 9 ABSENT.**
...
now **four** rows in two arcs whose status was wrong on the page rather than in the tree, in
both directions. Every row must name a declaration or say ABSENT *with a scope and a date*.
...
| 32 | `lem:n2-bounded` | 1788 | lemma | For any norm-continuous S1–S7 product on $\Mnsa{2}$, $M:=\sup_{n\in S^2}\lvert\widetilde t(n)\rvert<\infty$. | ABSENT — but no longer for the reason given before 2026-08-08: the frame function `n2FrameTwist` now exists, so this is the open statement that it is **bounded**, not a statement about a nonexistent map. **Priced 2026-08-08 (ARC-6 rung 6.1b), and it is the gate for the whole rest of this lane.** The article's proof is a contradiction: if $|\widetilde t(n_k)|\to\infty$, take $s_k = \pi/|\widetilde t(n_k)|\to 0$ and base points $a_k\to\lambda\id$; then $\Theta_{a_k}\to\Theta_{\lambda\id}=\id$, yet the rotation angle is exactly $\pm\pi$, so $\|\Theta_{a_k}-\id\|\geq 2$. The single missing input is **operator-norm continuity of $a\mapsto\Theta_a$ in the matrix argument** (S2 gives $L_a$ pointwise, finite dimension upgrades pointwise to operator norm, and $a\mapsto Q_{a^{-1/2}}$ is the functional calculus). ★ **That input is NOT missing from Mathlib** — `continuousOn_cfc` (plus `ContinuousAt.cfc`, `Filter.Tendsto.cfc`) in `Mathlib/Analysis/CStarAlgebra/ContinuousFunctionalCalculus/Continuity.lean` gives continuity of `cfc f` on elements with spectrum in a compact set, and this tree's carrier already uses `HermitianMat.cfc`. So this row is ABSENT-with-a-named-route, never BLOCKED | 6.1b banked |
