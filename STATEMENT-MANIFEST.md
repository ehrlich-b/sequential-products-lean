...
| 5 | `lem:span` | 418 | lemma | In an order unit space the effects contain the ball of radius $\tfrac12$ about $\tfrac12\id$, hence span $V$, so linear maps agreeing on effects are equal; likewise $[0,q]$ spans the Peirce subalgebra $J_2(q)$. | PARTIAL (span + the linear-map-extensionality use on the concrete carrier: `Necessity.span_isEffect_eq_top` with `LinearMap.ext_on`; the abstract-OUS version and the Peirce clause open) | 5.1 |
