...
narrative of arcs 2 and 3, followed by the M1–M7 milestone discharge records.

**Nothing here is current orders.** Read it for provenance only — what was tried,
in what order, and which framings were superseded. The live documents are:

* `LEDGER.md` — current orders, arc records, and the six-targets table;
* `THEOREM-MAP.md` — **the governing honesty ledger**: what the tree IS, per
  paper statement. When this archive and THEOREM-MAP disagree, THEOREM-MAP wins,
  and so does anything in `LEDGER.md`.

Module counts, job counts, and census numbers quoted below are as-of their own
entries and are all stale by construction.
