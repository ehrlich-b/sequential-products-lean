...
/-!
# The concrete `ComparisonSetup` on `H_n(𝕜)`  (campaign LEDGER 2.6/2.8 wiring)

This module instantiates the master-chain interface `MasterTheorem.ComparisonSetup`
on the concrete carrier `HermitianMat (Fin N) 𝕜` from an S1–S7 sequential product.
Every interface field except `Θ_jordan` (Kadison/van Imhoff–Roelands rigidity,
campaign M3, taken here as the single hypothesis `ThetaPreservesJordanG`) is
discharged by a machine-checked theorem of the necessity development.

* `jordanBilinG 𝕜` — the Jordan product `∘` as an ℝ-bilinear map.
* `opCommute_iff_commuteG` — **the FK/vdW compatibility bridge, PROVED on `H_n(𝕜)`**:
...
* `theta_fix_generalG` — the **span-extended** vdW 5.5: `Θ_a` fixes every Hermitian
...
* `thetaNormG` — Θ made total on invertible (PosDef) base points by normalization
  `Θ_a := Θ_{(‖a‖+1)⁻¹ • a}`; the 2.3 law `theta_smul` makes the normalization
  invisible on effects, which is exactly why vdW 5.4 exists.
* `comparisonSetupG` — the instance. Inputs: rank `3 ≤ N`, S2
  (`FirstArgContinuous`), and `ThetaPreservesJordanG` (the M3 import).
-/
...
/-- The Euclidean Jordan product `x ∘ y = ½(xy + yx)` on `H_n(𝕜)` as an
...
matrix; over ANY `RCLike` field the real diagonals and symmetric off-diagonals force
`[a,b]` to be a scalar, and a commutator is traceless, so it vanishes
(`HermitianMat.eq_zero_of_commute_hermitian_of_trace_zero`). -/

/-- The quarter identity: `(L_a L_b − L_b L_a) y = ¼ ([a,b] y − y [a,b])`. -/
theorem symmMul_symmMul_sub_matG (a b y : HermitianMat n 𝕜) :
    (a.symmMul (b.symmMul y) - b.symmMul (a.symmMul y)).mat
      = (4 : 𝕜)⁻¹ • ((a.mat * b.mat - b.mat * a.mat) * y.mat
...
theorem opCommute_pointwiseG {a b : HermitianMat n 𝕜}
    (h : OpCommute (jordanBilinG (n := n) 𝕜) a b) (y : HermitianMat n 𝕜) :
    a.symmMul (b.symmMul y) = b.symmMul (a.symmMul y) := by
  have := LinearMap.congr_fun h y
  simpa [mulOp, jordanBilin_applyG] using this

theorem single_conjTransposeG (i j : n) :
    (Matrix.single i j (1 : 𝕜))ᴴ = Matrix.single j i (1 : 𝕜) := by
  ext p q
...
`H_n(𝕜)` forces matrix commutation. -/
theorem commute_of_opCommuteG {a b : HermitianMat n 𝕜}
    (h : OpCommute (jordanBilinG (n := n) 𝕜) a b) :
    Commute a.mat b.mat := by
  set C : Matrix n n 𝕜 := a.mat * b.mat - b.mat * a.mat with hC
  -- step 1: the commutator commutes with every Hermitian matrix (quarter identity)
  have hcomm : ∀ y : HermitianMat n 𝕜, Commute C y.mat := by
    intro y
    have h4 := symmMul_symmMul_sub_matG a b y
    rw [opCommute_pointwiseG h y, sub_self] at h4
    have hz : C * y.mat - y.mat * C = 0 := by
      have h0 : (0 : Matrix n n 𝕜) = (4 : 𝕜)⁻¹ • (C * y.mat - y.mat * C) := by
        rw [← h4, HermitianMat.mat_zero]
      rcases smul_eq_zero.mp h0.symm with hbad | hgood
      · exact absurd hbad (by norm_num)
      · exact hgood
    show C * y.mat = y.mat * C
    exact sub_eq_zero.mp hz
  -- step 2: a commutator is traceless
  have htr : C.trace = 0 := by
    rw [hC, Matrix.trace_sub, Matrix.trace_mul_comm a.mat b.mat, sub_self]
  -- step 3: the field-uniform kill (no `I`-generators; see Hermitian/CommutantHermitian)
  rcases isEmpty_or_nonempty n with hemp | hne
  · show a.mat * b.mat = b.mat * a.mat
    ext i j
    exact isEmptyElim i
  · have hC0 : C = 0 :=
      HermitianMat.eq_zero_of_commute_hermitian_of_trace_zero hcomm htr
    show a.mat * b.mat = b.mat * a.mat
...
theorem opCommute_of_commuteG {a b : HermitianMat n 𝕜}
    (hab : Commute a.mat b.mat) :
    OpCommute (jordanBilinG (n := n) 𝕜) a b := by
  apply LinearMap.ext
  intro y
  simp only [mulOp, LinearMap.comp_apply, jordanBilin_applyG]
  exact symmMul_opCommute_of_commute hab y

/-- **The FK/vdW compatibility bridge on `H_n(𝕜)`, machine-checked**: Jordan-operator
...
theorem opCommute_iff_commuteG {a b : HermitianMat n 𝕜} :
    OpCommute (jordanBilinG (n := n) 𝕜) a b ↔ Commute a.mat b.mat :=
  ⟨commute_of_opCommuteG, opCommute_of_commuteG⟩
...
scalar by linearity. -/
theorem theta_fix_nonnegG (hS2 : P.FirstArgContinuous) {a : HermitianMat n 𝕜}
    (ha : IsEffect a) (hbd : a.mat.PosDef) {c : HermitianMat n 𝕜}
  ...
  rw [map_smul] at hfix
  exact smul_right_injective (HermitianMat n 𝕜) (ne_of_gt hpos) hfix
...
theorem theta_fix_generalG (hS2 : P.FirstArgContinuous) {a : HermitianMat n 𝕜}
    (ha : IsEffect a) (hbd : a.mat.PosDef) (b : HermitianMat n 𝕜)
    (hab : Commute a.mat b.mat) :
  ...
  calc theta P ha hbd b
    ...
    _ = b⁺ - b⁻ := by
        rw [theta_fix_nonnegG P hS2 ha hbd (HermitianMat.posPart_nonneg b) hplus,
          theta_fix_nonnegG P hS2 ha hbd (HermitianMat.negPart_nonneg b) hminus]
    _ = b := HermitianMat.posPart_add_negPart b

/-! ## Θ made total: `thetaNormG` -/

theorem posDef_norm_smulG {a : HermitianMat n 𝕜} (h : a.mat.PosDef) :
    (((‖a‖ + 1)⁻¹ • a).mat).PosDef := by
  rw [HermitianMat.mat_smul]
  exact h.smul (by positivity)

theorem isEffect_norm_smulG {a : HermitianMat n 𝕜} (h : a.mat.PosDef) :
    IsEffect ((‖a‖ + 1)⁻¹ • a) :=
...
`thetaNormG` agrees with `theta` (`thetaNorm_apply_eq_thetaG`). -/
def thetaNormG (hS2 : P.FirstArgContinuous) (a : HermitianMat n 𝕜) :
    HermitianMat n 𝕜 ≃ₗ[ℝ] HermitianMat n 𝕜 :=
  letI := Classical.dec (a.mat.PosDef)
  if h : a.mat.PosDef then
    thetaEquiv P hS2 (isEffect_norm_smulG h) (posDef_norm_smulG h)
  else LinearEquiv.refl ℝ _

theorem thetaNorm_of_posDefG (hS2 : P.FirstArgContinuous) {a : HermitianMat n 𝕜}
    (h : a.mat.PosDef) :
    thetaNormG P hS2 a = thetaEquiv P hS2 (isEffect_norm_smulG h) (posDef_norm_smulG h) := by
  unfold thetaNormG
  exact dif_pos h

/-- On effect base points the normalization is invisible (`theta_smul`). -/
theorem thetaNorm_apply_eq_thetaG (hS2 : P.FirstArgContinuous) {a : HermitianMat n 𝕜}
    (ha : IsEffect a) (hbd : a.mat.PosDef) (x : HermitianMat n 𝕜) :
    thetaNormG P hS2 a x = theta P ha hbd x := by
  have ht1 : (‖a‖ + 1)⁻¹ ≤ 1 := by
    have h0 : (0 : ℝ) < ‖a‖ + 1 := by positivity
    have h1 : (1 : ℝ) / (‖a‖ + 1) ≤ 1 :=
      (div_le_one h0).mpr (by linarith [norm_nonneg a])
    simpa [one_div] using h1
  rw [thetaNorm_of_posDefG P hS2 hbd, thetaEquiv_apply,
    theta_smul P hS2 ha hbd (by positivity) ht1
      (isEffect_norm_smulG hbd) (posDef_norm_smulG hbd)]

theorem thetaNorm_oneG (hS2 : P.FirstArgContinuous) {a : HermitianMat n 𝕜}
    (h : a.mat.PosDef) : thetaNormG P hS2 a 1 = 1 := by
  rw [thetaNorm_of_posDefG P hS2 h, thetaEquiv_apply, theta_one]

theorem thetaNorm_nonneg_iffG (hS2 : P.FirstArgContinuous) {a : HermitianMat n 𝕜}
    (h : a.mat.PosDef) (x : HermitianMat n 𝕜) :
    0 ≤ x ↔ 0 ≤ thetaNormG P hS2 a x := by
  rw [thetaNorm_of_posDefG P hS2 h, thetaEquiv_apply]
  exact (theta_nonneg_iff P hS2 _ _ x).symm

theorem thetaNorm_fixG (hS2 : P.FirstArgContinuous) {a : HermitianMat n 𝕜}
    (h : a.mat.PosDef) (b : HermitianMat n 𝕜)
    (hab : OpCommute (jordanBilinG (n := n) 𝕜) a b) :
    thetaNormG P hS2 a b = b := by
  have hc : Commute a.mat b.mat := commute_of_opCommuteG hab
  rw [thetaNorm_of_posDefG P hS2 h, thetaEquiv_apply]
  apply theta_fix_generalG P hS2 _ _ b
  rw [HermitianMat.mat_smul]
...
def ThetaPreservesJordanG : Prop :=
  ∀ {a : HermitianMat n 𝕜} (ha : IsEffect a) (hbd : a.mat.PosDef),
    PreservesJordan (theta P ha hbd)

theorem thetaNorm_jordanG (hS2 : P.FirstArgContinuous) (hjord : ThetaPreservesJordanG P)
    {a : HermitianMat n 𝕜} (h : a.mat.PosDef) (x y : HermitianMat n 𝕜) :
    thetaNormG P hS2 a (jordanBilinG 𝕜 x y)
      = jordanBilinG 𝕜 (thetaNormG P hS2 a x) (thetaNormG P hS2 a y) := by
  rw [thetaNorm_of_posDefG P hS2 h]
  simp only [thetaEquiv_apply, jordanBilin_applyG]
  exact hjord (isEffect_norm_smulG h) (posDef_norm_smulG h) x y

/-- The χ̃ cocycle transported to `thetaNormG` (LinearEquiv form, `.trans` order). -/
theorem thetaNorm_cocycleG (hS2 : P.FirstArgContinuous) (hjord : ThetaPreservesJordanG P)
    {r r' : n → ℝ} (hr : ∀ i, r i ≤ 0) (hr' : ∀ i, r' i ≤ 0) :
    thetaNormG P hS2 (diagFamilyG 𝕜 (r + r'))
      = (thetaNormG P hS2 (diagFamilyG 𝕜 r)).trans (thetaNormG P hS2 (diagFamilyG 𝕜 r')) := by
  have hsum : ∀ i, (r + r') i ≤ 0 := fun i => add_nonpos (hr i) (hr' i)
  have hsum' : ∀ i, (r' + r) i ≤ 0 := fun i => add_nonpos (hr' i) (hr i)
  apply LinearEquiv.ext
  intro x
  rw [LinearEquiv.trans_apply]
  rw [thetaNorm_apply_eq_thetaG P hS2 (diagFamilyG_isEffect hr) (diagFamilyG_posDef r) x]
  rw [thetaNorm_apply_eq_thetaG P hS2 (diagFamilyG_isEffect hr') (diagFamilyG_posDef r')]
  rw [thetaNorm_apply_eq_thetaG P hS2
    (diagFamilyG_isEffect hsum) (diagFamilyG_posDef (r + r')) x]
  have hcongr : theta P (diagFamilyG_isEffect hsum) (diagFamilyG_posDef (r + r'))
      = theta P (diagFamilyG_isEffect hsum') (diagFamilyG_posDef (r' + r)) :=
    theta_congrG P (by rw [add_comm]) _ _ _ _
  have hm := thetaD_mulG P hS2 hr' hr
    (hjord (diagFamilyG_isEffect hr') (diagFamilyG_posDef r'))
  rw [hcongr, hm]
  rfl

/-! ## The instance -/

theorem one_symmMulG (x : HermitianMat n 𝕜) :
    (1 : HermitianMat n 𝕜).symmMul x = x := by
  rw [HermitianMat.symmMul_comm, HermitianMat.symmMul_one]

/-- **The concrete `ComparisonSetup` on `H_N(𝕜)`.** Every field is discharged by a
machine-checked theorem of this development, except the single cited import
`Θ_jordan`, taken as the hypothesis `hjord : ThetaPreservesJordanG P` (van
...
def comparisonSetupG {N : ℕ} (hN : 3 ≤ N)
  ...
  aOf := diagFamilyG 𝕜
  aOf_inv := diagFamilyG_posDef
  Θ := thetaNormG P hS2
  Θ_unital := fun _ h => thetaNorm_oneG P hS2 h
  Θ_orderIso := fun _ h x => thetaNorm_nonneg_iffG P hS2 h x
  Θ_jordan := fun _ h x y => thetaNorm_jordanG P hS2 hjord h x y
  Θ_fix := fun _ h b hb => thetaNorm_fixG P hS2 h b hb
  frame_opCommute := fun r i => opCommute_of_commuteG (diagFamilyG_commute_frameProj r i)
  Θ_cocycle := fun _ _ hr hr' => thetaNorm_cocycleG P hS2 hjord hr hr'
