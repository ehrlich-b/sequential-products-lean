...
For a positive-definite effect `b` on `H_n(ℂ)` and an arbitrary S1–S7+S2 product:
...
theorem eigFinset_pos {b : HermitianMat n ℂ} (hbd : b.mat.PosDef) :
...
def pseudoInvCoef (b : HermitianMat n ℂ) : ℝ := ∏ μ ∈ b.eigFinset, μ

/-- The normalized spectral pseudo-inverse `∑_μ (c/μ) • P_μ`. -/
def pseudoInv (b : HermitianMat n ℂ) : HermitianMat n ℂ :=
  ∑ μ ∈ b.eigFinset, (pseudoInvCoef b / μ) • b.specProj μ

variable {b : HermitianMat n ℂ}
...
theorem pseudoInv_isEffect (hb : IsEffect b) (hbd : b.mat.PosDef) :
    IsEffect (pseudoInv b) :=
  sum_smul_proj_isEffect (fun μ _ => specProj_isProjection b μ)
...
theorem sp_pseudoInv_eq_smul_one (hS2 : P.FirstArgContinuous) (hb : IsEffect b)
  ...
  have hval := sp_orthFamily_value P hS2 (fun μ _ => specProj_isProjection b μ)
    (fun _ _ _ _ hμν => HermitianMat.specProj_mul_orth b hμν)
    (pseudoInvCoef_div_nonneg hbd) (pseudoInvCoef_div_le_one hb hbd)
...
theorem sp_pseudoInv_comm (hS2 : P.FirstArgContinuous) (hb : IsEffect b)
  ...
  have hcomm := sp_orthFamily_comm P hS2 (fun μ _ => specProj_isProjection b μ)
    (fun _ _ _ _ hμν => HermitianMat.specProj_mul_orth b hμν)
    (pseudoInvCoef_div_nonneg hbd) (pseudoInvCoef_div_le_one hb hbd)
    (fun μ hμ => HermitianMat.eigFinset_nonneg hb.1 μ hμ) (eigFinset_le_one hb.2)
...
theorem sp_pseudoInv_cancel (hS2 : P.FirstArgContinuous) (hb : IsEffect b)
    (hbd : b.mat.PosDef) {x : HermitianMat n ℂ} (hx : IsEffect x) :
  ...
  have hu : P.sp (1 : HermitianMat n ℂ) x = x := P.sp_unit_left hx
  rw [hu]
...
theorem span_isEffect_eq_top :
    Submodule.span ℝ {a : HermitianMat n ℂ | IsEffect a} = ⊤ := by
  rw [eq_top_iff]
  intro x _
  have hmem : ∀ y : HermitianMat n ℂ, 0 ≤ y →
      y ∈ Submodule.span ℝ {a : HermitianMat n ℂ | IsEffect a} := by
    intro y hy
...
theorem seqLeftMul_pseudoInv_comp (hS2 : P.FirstArgContinuous) (hb : IsEffect b)
  ...
  apply LinearMap.ext_on (s := {a : HermitianMat n ℂ | IsEffect a}) span_isEffect_eq_top
  intro e he
...
theorem seqLeftMul_reflectsNonneg (hS2 : P.FirstArgContinuous) (hb : IsEffect b)
    (hbd : b.mat.PosDef) {x : HermitianMat n ℂ}
  ...
  have h2 : seqLeftMul P (pseudoInv b) (pseudoInv_isEffect hb hbd)
    ...
    have h := congrArg (fun f : HermitianMat n ℂ →ₗ[ℝ] HermitianMat n ℂ => f x)
...
theorem seqLeftMul_injective (hS2 : P.FirstArgContinuous) (hb : IsEffect b)
  ...
  intro x y hxy
  have h : ∀ z : HermitianMat n ℂ, seqLeftMul P (pseudoInv b) (pseudoInv_isEffect hb hbd)
      (seqLeftMul P b hb z) = pseudoInvCoef b • z := by
    intro z
    have h := congrArg (fun f : HermitianMat n ℂ →ₗ[ℝ] HermitianMat n ℂ => f z)
      (seqLeftMul_pseudoInv_comp P hS2 hb hbd)
    simpa using h
