...
theorem conj_eq_jordan (s y : HermitianMat n 𝕜) :
    y.conj s.mat
...
theorem sqrt_isEffect {a : HermitianMat n 𝕜} (ha : IsEffect a) :
    IsEffect (a.cfc Real.sqrt) := by
  constructor
  · rw [show (0 : HermitianMat n 𝕜) ≤ a.cfc Real.sqrt ↔
...
definiteness needed). -/
theorem sp_comm_of_commute (P : SequentialProductOn (HermitianMat n 𝕜))
    (hS2 : P.FirstArgContinuous) {a b : HermitianMat n 𝕜}
    (ha : IsEffect a) (hb : IsEffect b) (hab : Commute a.mat b.mat) :
    P.sp a b = P.sp b a := by
  have hcomm := sp_orthFamily_comm P hS2 (p := fun q => jointProj hab q)
    (fun q (_ : q ∈ a.eigFinset ×ˢ b.eigFinset) =>
...
◦'-compatible pairs. -/
theorem seqLeftMul_mul_of_commute (P : SequentialProductOn (HermitianMat n 𝕜))
    (hS2 : P.FirstArgContinuous) {a b m : HermitianMat n 𝕜}
    (ha : IsEffect a) (hb : IsEffect b) (hab : Commute a.mat b.mat)
...
theorem quadRepEquiv_mul_of_commute {a b m : HermitianMat n 𝕜}
  ...
  have h := HermitianMat.sqrt_mul_of_commute ha0 hb0 hab hmm
...
def PreservesJordan (T : HermitianMat n 𝕜 →ₗ[ℝ] HermitianMat n 𝕜) : Prop :=
  ∀ x y : HermitianMat n 𝕜, T (x.symmMul y) = (T x).symmMul (T y)
...
theorem theta_conj_exchange (P : SequentialProductOn (HermitianMat n 𝕜))
    (hS2 : P.FirstArgContinuous) {a b : HermitianMat n 𝕜}
    (ha : IsEffect a) (hb : IsEffect b) (hbdb : b.mat.PosDef)
    (hab : Commute a.mat b.mat)
    (hjord : PreservesJordan (theta P hb hbdb)) (y : HermitianMat n 𝕜) :
...
`Θ_m = Θ_a ∘ Θ_b`. -/
theorem theta_cocycle_of_preservesJordan (P : SequentialProductOn (HermitianMat n 𝕜))
    (hS2 : P.FirstArgContinuous) {a b m : HermitianMat n 𝕜}
    ...
    (hmm : m.mat = a.mat * b.mat)
    (hjord : PreservesJordan (theta P ha hbda)) (x : HermitianMat n 𝕜) :
...
theorem symmMul_opCommute_of_commute {a b : HermitianMat n 𝕜}
    (hab : Commute a.mat b.mat) (x : HermitianMat n 𝕜) :
...
theorem conj_eq_self_of_symmMul_eq_self {e x : HermitianMat n 𝕜}
  ...
  have hleft : e.mat * x.mat * e.mat = e.mat * x.mat := by
    ...
    have h2 : e.mat * x.mat + e.mat * x.mat * e.mat
      ...
      calc e.mat * x.mat + e.mat * x.mat * e.mat
          = (2 : 𝕜) • (e.mat * x.mat) := h
        _ = e.mat * x.mat + e.mat * x.mat := two_smul 𝕜 _
  ...
  have hright : e.mat * (x.mat * e.mat) = x.mat * e.mat := by
    ...
    have h2 : e.mat * (x.mat * e.mat) + x.mat * e.mat
      ...
      calc e.mat * (x.mat * e.mat) + x.mat * e.mat
          = (2 : 𝕜) • (x.mat * e.mat) := h
        _ = x.mat * e.mat + x.mat * e.mat := two_smul 𝕜 _
  ...
  have h2x : e.mat * x.mat + e.mat * x.mat = (2 : 𝕜) • x.mat := by
    rw [← hmat, hcomm]
  calc e.mat * x.mat * e.mat
      = e.mat * x.mat := hleft
    _ = x.mat := by
      have h3 := h2x
      rw [← two_smul 𝕜 (e.mat * x.mat)] at h3
